...
internal class MessageMover(
    ...
    private val messageThreader: MessageThreader
) {
    fun moveMessage(messageId: Long, destinationFolderId: Long): Long {
        ...
        return database.execute(true) { database ->
            val threadInfo = messageThreader.createOrUpdateParentThreadEntries(database, messageId, destinationFolderId)
            val destinationMessageId = createMessageEntry(database, messageId, destinationFolderId, threadInfo)
            messageThreader.createOrUpdateThreadEntry(database, destinationMessageId, threadInfo)
