...
class MessageMoverTest : RobolectricTest() {
    val sqliteDatabase = createDatabase()
    val lockableDatabase = mock<LockableDatabase> {
        on { execute(anyBoolean(), any<DbCallback<Any>>()) } doAnswer { stubbing ->
            val callback: DbCallback<Any> = stubbing.getArgument(1)
    ...
    fun `move message not part of a thread`() {
        ...
        val messageMover = MessageMover(lockableDatabase, messageThreader)

        val destinationMessageId = messageMover.moveMessage(
    ...
    private fun createMessageThreader(threadInfo: ThreadInfo): MessageThreader {
        return mock {
